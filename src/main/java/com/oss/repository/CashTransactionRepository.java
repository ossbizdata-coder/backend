...
public interface CashTransactionRepository extends JpaRepository<CashTransaction, Long> {
    ...
    @Query("SELECT ct FROM CashTransaction ct " +
           "WHERE ct.type = 'EXPENSE' " +
           "AND ct.expenseType.id IN (100, 106) " +
           "AND ct.dailyCash.businessDate >= :startDate " +
           "AND ct.dailyCash.businessDate <= :endDate " +
           "ORDER BY ct.createdAt DESC")
    List<CashTransaction> findBankDepositsByDateRange(
            @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate
    );
    // Expenses by date range for reporting
    @Query("SELECT ct FROM CashTransaction ct " +
           "WHERE ct.type = 'EXPENSE' " +
           "AND ct.dailyCash.businessDate >= :startDate " +
           "AND ct.dailyCash.businessDate <= :endDate")
    List<CashTransaction> findExpensesByBusinessDateRange(
            @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate
    );
}